...
// These functios contain a lot of magic numbers representing offset data
// see the structural documents found in file.h to see how these are derived
...
static uint32_t get_cluster_offset(uint32_t index) {
    ...
    // index should always be greater than two, because 0 and 1 are reserved?
    ASSERT(index >= 2);

    struct BPB* p = get_fs_bpb();

    reserved_size = (uint32_t)p->reserved_sector_count * p->bytes_per_sector;
    ...
    return reserved_size + fat_size + dir_size +
...
// Number of entries in the root directory structure
static uint32_t get_root_directory_count(void) {
    struct BPB* bpb = get_fs_bpb();

    return (uint32_t)bpb->root_entry_count;
}
...
static struct DirEntry *get_root_directory(void) {
    ...
    p = get_fs_bpb();
    offset = (
        p->reserved_sector_count + 
        (uint32_t)p->fat_count * 
        p->sectors_per_fat) * 
        p->bytes_per_sector;
...
// Find name and extension file in full path
static bool split_path(char *path, char *name, char *ext) {
    int i;

    for (i = 0; i < 8 && path[i] != '.' && path[i] != '\0'; ++i) {
        // We don't support '/' in filenames or extensions
        if (path[i] == '/') {
            return false;
        }
    ...
    if (path[i] == '.') {
        // . isn't part of the name, just a field separator
        ++i;
...
// Find beginning of data for file with the specified path
static uint32_t search_file(char *path) {
    ...
    if (status == true) {
        ...
        for (uint32_t i = 0; i < root_entry_count; ++i) {
            ...
            // We don't support FAT32 long file names, so pretend they don't exist
    ...
    // We didn't find the desired file name
    return 0xfffffff;
}
...
static uint32_t read_raw_data(uint32_t cluster_index, char *buffer, uint32_t size) {
    ...
    // index should always be greater than two, because 0 and 1 are reserved
    // So return and claim the file didn't exist.
    if (index < 2) {
        return 0xffffffff;
    }
    ...
    while (read_size < size) {
        ...
        // We're almost at the end of the data, so copy cluster remainder
        // rather than a full cluster
        if (index >= 0xfff7) {
...
static uint32_t read_file(uint32_t cluster_index, char *buffer, uint32_t size) {
...
int load_file(char *path, uint64_t addr) {
    ...
    // I guess 0xffffffff indicates we didn't find the file
    if (index != 0xffffffff) {
        dir_entry = get_root_directory();
